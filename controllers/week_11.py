...
LINK2 = "https://nuscs2113-ay1819s1.github.io/website/admin/project-w11-v13.html"
TIMEDELTA = timedelta(days=1, hours=2) # 2-am checking # Set  timedelta(days=1) for CS2103
TIMEDELTA_MILESTONE = timedelta(days=1, hours=12) # next day checking # Set timedelta(hours=1) for CS2103
PRODUCTION = False
##############################################################
...
OUTPUT_DIR = "./output/"
CSV_HEADER = ["Student", "Team", "Team_Repo", "Team_PR", "Auto_Publish", "Travis", "Jar", "README_modified", "README_ack",\
        ...
        "Student_PR", "DG", "UG", "AboutUs", "README", "ContactUs", "Java", "{}_issue_assigned".format(MILESTONES[0])]
...
class Week_11(BaseController):
            ...
            if team in self.team_repo_mapping.keys():
                ...
                for release in repository.get_releases():
                    for asset in release.get_assets():
                        if (JAR in asset.name.lower()) and (asset.updated_at >= (self.end_datetime-timedelta(days=8))):
    ...
    def check_travis_build_passing(self, teams_to_check):
        ...
        for team, students in teams_to_check.items() :
            ...
            for build in builds :
                if (build.commit.tag == MILESTONES[0]):
                    try:
