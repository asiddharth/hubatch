...
LINK2 = "https://nuscs2113-ay1819s1.github.io/website/admin/project-w09-v12.html"
...
class Week_9(BaseController):
    ...
    def get_feedback_message(self, teams_to_check, tutor_map, audit_details, args, end_datetime):
        ...
        for team, students in teams_to_check.items():
            ...
            # All issues closed in v1.2 or not
            if int(audit_details["issue_closed_v1.2"][team_index]) == 0:
                team_feedback+=[" ", message["not_done"]]
            else:
                team_feedback+=[message["x_mark"], message["done"]]

            # milestone v1.2 closed or not
            if int(audit_details["v1.2_closed"][team_index]) == 0:
                team_feedback+=[" ", message["not_done"]]
            else:
                team_feedback+=[message["x_mark"], message["done"]]

            # code tag: v1.2
            if int(audit_details["v1.2_tagged"][team_index]) == 0:
                team_feedback+=[" ", message["not_done"]]
            else:
                team_feedback+=[message["x_mark"], message["done"]]
        ...
        for team, students in team_list.items():
            ...
            for student in students:
                ...
                if self.student_merged_docs[student] == 0:
                    self.student_merged_docs[student] = (int(self.audit_details_last["DG"][indiv_index]) or int(self.audit_details_last["UG"][indiv_index]) \
                                                        or int(self.audit_details_last["AboutUs"][indiv_index]) or int(self.audit_details_last["README"][indiv_index]))
            ...
            if team in self.team_repo_mapping.keys():
                ...
                for issue in repository.get_issues(state="all"):
                    ...
                    if (issue.milestone is not None) and (MILESTONES[0] in issue.milestone.title.lower()):
                        ...
                        # if any one issue assigned to 1.2 is open, then all issues are not closed/merged
